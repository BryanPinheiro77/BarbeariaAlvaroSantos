...
Aplicar db/migrations/002_booking_requests.sql antes do deploy. A migração é transacional e preserva todas as sobreposições antigas: não altera nem apaga agendamentos existentes. Um trigger passa a validar somente novas inclusões e alterações de data/horário. Não depende da migração de refresh token.
...
Lock transacional PostgreSQL por chave e por dia serializa criação de cliente e admin entre instâncias. Um trigger com o mesmo advisory lock impede sobreposição por outros escritores sem validar retroativamente os registros antigos. Intervalos adjacentes são permitidos; cancelados liberam horário. A agenda atual é única. Reservas atravessando meia-noite são rejeitadas. Idempotência engloba criação de agendamento; cobranças no provedor de pagamento são operação separada.

Aceite: a migração aplica mesmo com duplicidades históricas; duas requisições com a mesma chave resultam no mesmo ID; duas chaves para intervalo sobreposto resultam em um sucesso e um 409; rollback não deixa chave presa; tentativas não dependem da memória de uma JVM. Teste PostgreSQL: BOOKING_TEST_DB_URL=jdbc:postgresql://localhost:PORT/test BOOKING_TEST_DB_USER=test BOOKING_TEST_DB_PASSWORD=test bash mvnw -Dtest=BookingRequestsTest test. Usar banco descartável: o teste cria e remove somente seu schema aleatório.
